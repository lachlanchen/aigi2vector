#!/usr/bin/env python3
"""Crop elements from page images using layout JSON files (v2 naming)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--only", help="Only process one page (basename without extension)")
...
def main() -> None:
    ...
    json_paths = sorted(layout_dir.glob("*.json"))
    if args.only:
        json_paths = [layout_dir / f"{args.only}.json"]

    for json_path in json_paths:
        if not json_path.exists():
            print(f"Missing layout file: {json_path}")
            continue
        ...
        with Image.open(image_path) as img:
            ...
            for idx, el in enumerate(elements, start=1):
                ...
                name = f"{idx:03d}_x{int(x):04d}_y{int(y):04d}_{notes}.png"
