...
usage() {
  cat <<'USAGE'
Usage: codex_extract_layout_elements.sh --input-dir <images> --md-dir <markdown> --output-dir <path> [options]
...
for image in "${images[@]}"; do
  ...
  cat > "$prompt_file" <<EOF
Goal: Extract a dense, detailed set of atomic visual elements with bounding boxes.
Current task: Use the image plus the provided markdown to identify text blocks, icons, arrows, diagrams, photos, and other distinct elements. Provide coordinates for each element.
...
- Include all visible text elements as separate entries when possible.
- Use "text" type for text elements; put the exact text in the "text" field.
